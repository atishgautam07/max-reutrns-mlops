...
@ensure_annotations
def create_gcs_directories(bucket_name: str, prefixes: list):
    """Creates 'directories' in a GCS bucket by uploading a placeholder file."""
    storage_client = storage.Client()
    bucket = storage_client.bucket(bucket_name)
    for prefix in prefixes:
        if not prefix.endswith('/'):
            prefix += '/'
        blob = bucket.blob(prefix + 'placeholder.txt')
        blob.upload_from_string('')
        print(f"Created GCS path: {prefix}")